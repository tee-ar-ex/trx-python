...
from nibabel.streamlines.tractogram import TractogramItem, Tractogram
import numpy as np
import pytest
...
@pytest.mark.parametrize("filename,expected_base,expected_ext", [
...
def test_get_reference_info_wrapper_nifti_header(nifti_ref):
    ...
    affine, dimensions, voxel_sizes, voxel_order = get_reference_info_wrapper(nifti_ref.header)
    assert np.allclose(affine, nifti_ref.affine)
    assert np.array_equal(dimensions, [10, 20, 30])
...
def test_get_reference_info_wrapper_trk_file(mock_load):
    ...
    affine, dimensions, voxel_sizes, voxel_order = get_reference_info_wrapper("test.trk")
...
def test_get_reference_info_wrapper_trk_dict(trk_header):
    ...
    affine, dimensions, voxel_sizes, voxel_order = get_reference_info_wrapper(trk_header)
...
def test_get_reference_info_wrapper_trx_file(mock_load):
    ...
    affine, dimensions, voxel_sizes, voxel_order = get_reference_info_wrapper("test.trx")
    assert np.allclose(affine, mock_trx.header['VOXEL_TO_RASMM'])
...
def test_get_reference_info_wrapper_zero_affine():
    ...
    mock_header.__getitem__.side_effect = lambda x: [10, 10, 10] if x == 'dim' else [1, 1, 1]
...
def test_get_reference_info_wrapper_error():
    ...
    with pytest.raises(TypeError, match="Input reference is not one of the supported format"):
...
@pytest.mark.skipif(not dipy_available, reason="Dipy is not installed.")
@pytest.mark.parametrize("space_str,origin_str,expected_space,expected_origin", [
